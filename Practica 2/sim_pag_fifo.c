...
int elegir_pagina_para_reemplazo (ssistema * S)
{
    int marco, victima;
    int ultimo;
    ultimo=S->listaocupados;	//Almacenamos en ultimo, el ultimo marco ocupado
    marco=S->tdm[ultimo].sig;	//El siguiente marco ocupado, es el primero
    victima = S->tdm[marco].pagina;	//La victima sera la pagina del primer marco ocupado
    S->listaocupados=marco;		//El primer marco, pasa a ser el ultimo marco de la lista ocupado
    if (S->detallado)
...
void reemplazar_pagina (ssistema * S, int victima, int nueva)
{
    ...
    marco = S->tdp[victima].marco;	//Almacenamos en marco, el marco de la pagina a reemplazar
...
void ocupar_marco_libre (ssistema * S, int marco, int pagina)
{
    ...
    int primero,ultimo;
    ultimo=S->listaocupados;//Almacenamos en ultimo, el ultimo marco de la listaocupados
    primero=S->tdm[ultimo].sig;//Primero, es el siguiente del ultimo marco de la listaocupados, debido a que es una lista circular
    S->tdm[marco].sig=primero;//El siguiente marco del marco introducido, sera el primero
    S->tdm[ultimo].sig=marco;//EL siguiente marco del ultimo marco de la lista, sera el nuevo introducido
    S->listaocupados=marco;//Ponemos en ultima posicion de la listaocupados al marco introducido
}
...
{
    int ultimo,aux;
    printf("Reemplazo fifo Marcos Ocupados acutualmente: ");
    ultimo=S->listaocupados;
    aux=S->tdm[ultimo].sig;
    while(aux!=ultimo)//Mientas que la el aux no sea ultimo mostrara por pantalla los marcos de la lista ocupados, cuando sea igual al ultimo, nos indica que ya la ha recorrido entera.
	{
	printf("%6d", aux);
	aux=S->tdm[aux].sig;
	}
    printf("%6d", aux);//Por último muestra por pantalla el último elemento.
}
